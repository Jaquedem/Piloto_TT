...
class InterpretIA_App(ctk.CTk):
    def __init__(self):
        ...
        self.last_sentence = ""  # Para trackear cambios de texto
            ...
            if ret:
                ...
                if self.frame_count % 3 == 0:
                    ...
                    if results[0].boxes:
                        ...
                        self.interpreter.process_detection(label_name)
                    else:
                        # B) NO detectó nada -> Enviamos 'None' para checar tiempos de silencio
                        self.interpreter.process_detection(None)

                # --- 2. ACTUALIZACIÓN DE TEXTO (En cada frame) ---
                # Obtenemos siempre la traducción actual del intérprete
                current_sentence = self.interpreter.get_current_translation()

                # Solo actualizamos la UI si el texto cambió (evita parpadeos)
                if current_sentence != self.last_sentence:
                    self.text_display.configure(text=current_sentence)
